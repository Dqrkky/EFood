...
class Efood:
            ...
            if data0 != None and isinstance(data0, dict):
                if "data" in data0 and data0["data"] != None and isinstance(data0["data"], dict):
                    ...
                    if "session_id" in data1 and data1["session_id"] != None and isinstance(data1["session_id"], str):
                        self.session.headers["User-Agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.0.0 Safari/537.36 Edg/106.0.1370.42"
                        self.session.headers["x-core-session-id"] = data0["data"]["session_id"]
                        self.session.headers["X-core-platform"] = "web"
                        self.session.headers["X-core-version"] = "2.31.2"
                        self.session.headers["Accept-Language"] = "el"
    ...
    def reset_password(self, email :str=None):
        if email != None and isinstance(email, str) and email != "" and email != " ":
            ...
            if data0 != None and isinstance(data0, dict):
                if "status" in data0 and data0["status"] != None and isinstance(data0["status"], str) and data0["status"] != "" and data0["status"] != " ":
